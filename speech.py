...
def lpc_analysis_1(X, order=8, window_step=128, window_size=2 * 128,
...
def lpc_analysis(X, order=8, window_step=128, window_size=2 * 128,
                 emphasis=0.9, overlap_add=True, copy=True):
    """
    Extract LPC coefficients from a signal
    Based on code from:
        http://labrosa.ee.columbia.edu/matlab/sws/
    ...
    # Pad to even number of windows
    n_points = X.shape[1]
    # Bug in MATLAB code? Why would you calculate this *before* padding?
    n_windows = X.shape[1] // window_step
    # Bug number 2? Why are these two different sizes?
    pad_sizes = [(window_size - window_step) // 2,
                 window_size - window_step // 2]
    # TODO: Handling for odd window sizes / steps
    X = np.hstack((np.zeros((X.shape[0], pad_sizes[0])), X,
    ...
    if overlap_add:
        ...
        residual_excitation = np.zeros((n_points))
    # Pre-emphasis high-pass filter
    X = sg.lfilter([1, -emphasis], 1, X)
    # stride_tricks.as_strided
    # eventually handle multiple input signals
    for window in range(n_windows - 1):
        XX = X.ravel()
        XX = XX[window * window_step + np.arange(window_size)]
        WXX = XX * sg.hanning(window_size)
        RXX = np.correlate(WXX, WXX, mode='full')
        RXX = RXX[np.arange(window_size - 1, window_size + order)]
        R = linalg.toeplitz(RXX[:-1])
        solved_R = linalg.pinv(R).dot(RXX[1:])
        filter_coefs = np.hstack((1, -solved_R))
        if overlap_add:
            ...
            residual_signal = sg.lfilter(
                filter_coefs, 1, XX[pad_sizes[0] + np.arange(window_step)])
        gain = np.sqrt(np.mean(residual_signal ** 2))
        lp_coefficients[window] = filter_coefs
        per_frame_gain[window] = gain
        if overlap_add:
            assign_range = window * window_step + np.arange(window_size)
            additional = residual_excitation[assign_range] + \
                residual_signal / gain
            residual_excitation[assign_range] += additional
        else:
            assign_range = window * window_step + np.arange(window_step)
            residual_excitation[assign_range] = residual_signal / gain
    # Throw away first part in overlap mode for proper synthesis
    if overlap_add:
        residual_excitation = residual_excitation[pad_sizes[0]:]
...
def test_feature_build():
    ...
    samplerate, X = wavfile.read('/Users/aarontaub/Google Drive/AaronAndAmitBIU/FinalProject/Praat/whitney houston.wav')
