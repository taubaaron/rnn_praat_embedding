...
# Fit LSTM model
history = m.fit(lpc_in_matrix.T, lpc_out_matrix.T, epochs = 5, batch_size = 50,verbose=0)
...
xin = []
next_X1 = []
for i in range(window, len(lpc_test)):
    xin.append(lpc_test[i - window:i])
    next_X1.append(lpc_test[i])
